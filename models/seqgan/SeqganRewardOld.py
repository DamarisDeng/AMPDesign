import numpy as np
import pickle # can delete
import tensorflow as tf
from tensorflow.python.ops import tensor_array_ops, control_flow_ops
from tensorflow.keras.models import load_model

def convert(seq):
    """Converts input_x (number) to onehot encoding"""
    max = np.max(seq)
    X = np.zeros((len(seq),150,max+1))
    for i,m in enumerate(seq):
        for l,s in enumerate(m):
            X[i,l,s-1]=1
    return X

def predict_activity(input_x):
    model = load_model("/home/dengjunyang/Texygen/Ec.h5",compile=False)
    seq = convert(input_x)
...
class Reward(object):
    ...
    def get_reward(self, sess, input_x, rollout_num, discriminator): # 为什么把rollout_num设为16
        ...
        for i in range(rollout_num):
            ...
            print('input_x') # TODO
            with open('save/input_x.pkl', 'wb') as f:
                pickle.dump(input_x, f)
            predict_result = predict_activity(input_x) # new
            feed = {discriminator.input_x: input_x}
            ypred_for_auc = sess.run(discriminator.ypred_for_auc, feed)
            ypred = lambda_*np.array([item[1] for item in ypred_for_auc])+(1-lambda_)*np.array(predict_result) # new
            if i == 0:
                rewards.append(ypred)
            else:
